/** 
 * @fileoverview This file is used to predict the possible autocompletions of some word i n input field. As a whole this file handles task of suggesting and showing word suggestions. Variable starting from g are Global Variable. 
 * @author Chinmay Vaishampayan (chinmay@cse.iitb.ac.in)
 */

var gWords = new Array("saaii", "chinmaya", "satya", "sa.toora", "saara.gee", "tablaa", "veeNaa", "baa.sooree", "raamakRShNa");
...
function Predict() {
	...
	var converted; //converted to hindi
	var matches; //to check whether word to be predicted starts from letter or not.
	...
	if(matches) {
		gWords = gWords.sort();
		string = gWords.toString();
		string = "," + string + ",";
		gSuggestions = Suggest(word, string);
...
/**
 *It finds suggestions from stored words.
 *@author Chinmay Vaishampayan (chinmay@cse.iitb.ac.in)
 *@param {String} word Starting of word to be suggested.
 *@param {String} string Data from which suggestions will be provided. Should be Comma Separated String. Starting and ending with comma.
 *@returns Array containing suggestions
 *@type Array
 */
function Suggest(word, string) {
	var curMatch = 0; //Stores position of regexp, latest matched in string.
	var i = 0; //used for iteration in array.
	var nextDelim; //Search comma just after specified postition in each iteration.
	var numMatches; //Stores number of times the regexp matches.
	var matches
	var prevDelim; //Search comma just before specified postition in each iteration.
	var suggestions = new Array(); //To hold the word suggestions.

	var pattern = "," + word;
	pattern = Escape(pattern); //Escape special characters in Regular Expression like . ^ $
	var regexp = new RegExp(pattern, "g");
	regexp.test(string);
	
	curMatch = regexp.lastIndex - word.length-1; //using lastIndex property of RegExp
	...
	for(i=1; i<=numMatches; i++) {
		curMatch = string.indexOf(word, curMatch+1);
		prevDelim = string.lastIndexOf(',', curMatch-1);
		//if comma not found, then set it to start of string as that is the only place where prevDelim can be zero.
		if(prevDelim == -1) {
			prevDelim = 0;
		}
		nextDelim = string.indexOf(',', curMatch+1);
		
		//save suggestion only if it is not equal to word!!!.
		if(word != string.substring(prevDelim+1, nextDelim)) {
			suggestions.push(string.substring(prevDelim+1, nextDelim));
		}
	}
	return suggestions;
}
...
/**
 *It displays suggestions in browser.
 *@author Chinmay Vaishampayan (chinmay@cse.iitb.ac.in)
 *@param {HTML Element} element Element relative to which suggestion box will be displayed. In our case, input box.
 */
function ShowSuggest(element) {
	...
	if(gSuggestions.length >=1) {
		...
		left += (((lines[(lines.length) - 1]).length)%42)*8; //8 pixels horizontoly for one character
		top += (numLines)*17; //approx height of line in pixels.
	
		divSuggestBox  = document.createElement('div');
		divSuggestBox.setAttribute('id',"suggest"); //Set its ID
		divSuggestBox.style.left = left + "px";
		divSuggestBox.style.top = top + "px";
		
		document.getElementById("main").appendChild(divSuggestBox);
		...
		for(i in gSuggestions) {
			...
			textNode = document.createTextNode(Convert(gSuggestions[i]));
...
function HandleClickOnDiv() {
	...
	Complete(document.getElementById(id), document.getElementById("input"));
	return false;
}
...
function Complete(source, target) {
	...
	document.getElementById("output").value = Convert(leftPart + completeWord + rightPart);
	
	document.getElementById("main").removeChild(document.getElementById("suggest"));
	target.focus();
...
function SuggestionKeyDown(event) {
	...
	if(keyCode == 27) { //27 = escape key
		...
		Complete(this, document.getElementById("input"));
		return false;
	}
...
/**
 *Saves all new words from input textarea in suggestion database.At present it saves only temporarily in javascript array. After saving it calls function editSaveButton which is to show confirmation of saving of words.
 *@author Chinmay Vaishampayan (chinmay@cse.iitb.ac.in)
 *@param {HTML Element} btn Button which was clicked to call this fucntion.
 *@param {HTML Element} id Element from which words to be saved are taken.
 */
function SaveWords(btn, id) {
	var text = document.getElementById(id).value;
	var delimRegexp = /[-\s]/;
	var words = text.split(delimRegexp);
	var string = gWords.toString();
	var regexp;
	var i;
	var newWords=0; //counts of number of NEW words which were saved.
	var display;
	
	for(i in words) {
		regexp = new RegExp(Escape(words[i]), "g");
		if(!regexp.test(string)) {
			gWords.push(words[i]);
			newWords++;
		}
	}
	display = newWords + " new words saved";
	editSaveButton(btn, display); // in animate.js file
}